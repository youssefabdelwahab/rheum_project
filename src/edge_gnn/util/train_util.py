...
from src.edge_gnn.gnn.gat import GlobalGraphNetwork
from src.edge_gnn.loss.edge_info_loss import EdgeConditionedInfoNCE
...
# The train_pipeline function you just updated
from src.edge_gnn.train.custom_train_ce import train_pipeline
...
def prepare_dataset(dataset_name: str, data_dir: str, k_freq: int = 8):
    """
    Loads benchmark datasets and computes Laplacian eigenvectors for WIRE coordinates.
    """
    print(f"Loading {dataset_name} from: {data_dir}")
    
    if dataset_name == 'PubMed':
        dataset = Planetoid(root=os.path.join(data_dir, 'Planetoid'), name='PubMed')
    ...
    # ==========================================
    # Inject WIRE Coordinates here
    # ==========================================
    if getattr(data, 'wire_coords', None) is None:
        print(f"Calculating {k_freq} Laplacian eigenvectors for structural coordinates...")
        # Your function handles the undirected conversion and normalization automatically
        data.wire_coords = compute_eigenvectors(
            edge_index=data.edge_index, 
            num_nodes=data.num_nodes, 
            k_freq=k_freq
        )
        print("WIRE coordinates successfully generated.")
